...
public class MatchResolver {
  ...
  public MatchResolver() {
    _mock.Setup(x => x.GetItem(It.IsAny<AssetLocation>()))
        .Returns<AssetLocation>(Framework.Server.GetItem);
    _mock.Setup(x => x.SearchItems(It.IsAny<AssetLocation>()))
        .Returns<AssetLocation>(Framework.Server.SearchItems);
    _mock.SetupGet(x => x.Items).Returns(Framework.Server.World.Items);
    _mock.Setup(x => x.GetBlock(It.IsAny<AssetLocation>()))
        .Returns<AssetLocation>(Framework.Server.GetBlock);
    _mock.Setup(x => x.SearchBlocks(It.IsAny<AssetLocation>()))
        .Returns<AssetLocation>(Framework.Server.SearchBlocks);
    _mock.SetupGet(x => x.Blocks).Returns(Framework.Server.World.Blocks);

    _resolver = new(_mock.Object, Framework.Server.Api.Logger);
  }
  ...
  public void GetMatchingItemsWildcardStartWildcardDomain() {
    IReadOnlyList<Item> items =
        _resolver.GetMatchingItems(new AssetLocation("*", "*apple"));
    Assert.IsTrue(items.Any((item) => item.Code.Path == "fruit-pineapple"));
    Assert.IsTrue(items.All((block) => block.Code.Path.EndsWith("apple")));
  }
  ...
  public void GetMatchingBlocksWildcardStart() {
    Block chickenEgg = Framework.Server.World.GetBlock(
